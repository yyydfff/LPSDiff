...
def _parse_all_args():
    ...
    if default_opt.model_name not in model_dict:
        raise ValueError(
            f"Unknown model: {default_opt.model_name}. Available models: {list(model_dict.keys())}"
        )
...
def _smart_load_state_dict(module: nn.Module, ckpt_path: str, device):
    ...
    if isinstance(ckpt, dict) and "model" in ckpt:
        print(
            ...
            f"val_psnr={ckpt.get('val_psnr')} | "
            f"test_psnr={ckpt.get('test_psnr')}"
        )
        state = ckpt["model"]
    else:
        print("[Checkpoint Info] pure state_dict checkpoint")
        state = ckpt
    ...
    print(
        f"[Load] Loaded checkpoint from {ckpt_path}: {msg}"
    )
